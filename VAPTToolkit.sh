...
cookie_check() {
    ...
    while IFS= read -r LINE; do
        ...
        CNAME=$(echo "$CLEAN_LINE" | sed 's/^Set-Cookie: *//i' | cut -d'=' -f1)
        RAW_COOKIE=$(echo "$CLEAN_LINE" | sed 's/^Set-Cookie: *//i')
        echo -e "${CYAN}━━━ Cookie: ${RESET}$CNAME"
        echo -e "    ${YELLOW}Raw Header: ${RESET}$RAW_COOKIE"
        echo "$CLEAN_LINE" | grep -qi "Secure" && echo -e "  ${GREEN}[OK]${RESET} Secure" || { echo -e "  ${RED}[MISSING]${RESET} Secure flag"; ISSUES=$((ISSUES+1)); }
        echo "$CLEAN_LINE" | grep -qi "HttpOnly" && echo -e "  ${GREEN}[OK]${RESET} HttpOnly" || { echo -e "  ${RED}[MISSING]${RESET} HttpOnly flag"; ISSUES=$((ISSUES+1)); }
        if echo "$CLEAN_LINE" | grep -qi "SameSite"; then
            SV=$(echo "$CLEAN_LINE" | grep -oi "SameSite=[A-Za-z]*" | cut -d= -f2)
...
port_scan() {
    ...
    # Expanded port-to-service mapping
    get_service() {
        case $1 in
            ...
            1080) echo "SOCKS";; 1194) echo "OpenVPN";; 1433) echo "MSSQL";; 1521) echo "Oracle";;
            1723) echo "PPTP";; 2049) echo "NFS";; 2375) echo "Docker";; 2376) echo "Docker-TLS";;
            3306) echo "MySQL";; 3389) echo "RDP";; 4443) echo "HTTPS-Alt";; 5000) echo "Flask/Docker";;
            5432) echo "PostgreSQL";; 5601) echo "Kibana";; 5900) echo "VNC";; 6379) echo "Redis";;
            6443) echo "Kubernetes";; 8000) echo "HTTP-Alt";; 8080) echo "HTTP-Alt";;
            8081) echo "HTTP-Alt";; 8443) echo "HTTPS-Alt";; 8888) echo "Jupyter";;
            9090) echo "WebConsole";; 9200) echo "Elasticsearch";; 9300) echo "ES-Transport";;
            10000) echo "Webmin";; 11211) echo "Memcached";; 27017) echo "MongoDB";;
            27018) echo "MongoDB2";; 50000) echo "SAP";; *) echo "Unknown";;
        esac
    }

    # Banner grabbing helper
    grab_banner() {
        local target=$1 port=$2 banner=""
        # Try greeting banner (SSH, FTP, SMTP, etc.)
        banner=$(nc -w 1 -G 2 "$target" "$port" </dev/null 2>/dev/null \
            | tr -cd '\11\12\15\40-\176' | head -n 1 \
            | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
        # For web ports, grab the Server header
        if [ -z "$banner" ]; then
            case $port in
                80|443|8080|8443|8000|8081|4443)
                    banner=$(printf "HEAD / HTTP/1.0\r\n\r\n" \
                        | nc -w 1 -G 2 "$target" "$port" 2>/dev/null \
                        | grep -i "^Server:" | head -n 1 \
                        | sed 's/^Server: *//i' | tr -d '\r')
    ...
    if [ "$MODE" == "full" ]; then
        PORTS=(21 22 23 25 53 80 110 111 135 139 143 443 445 993 995 1433 1521 3306 3389 5432 5900 6379 8080 8443 8888 9090 27017 11211 2049 5000 5601 9200 9300 6443 2375 2376 4443 8000 8081 10000 50000 1080 389 636 161 162 514 1194 1723)
    else
    ...
    OPEN=0; TOTAL=${#PORTS[@]}; CURRENT=0
    for P in "${PORTS[@]}"; do
        CURRENT=$((CURRENT+1))
        SVC=$(get_service "$P")
        # Live progress indicator on a single updating line
        printf "\r${YELLOW}[SCANNING]${RESET} %-25s [%d/%d]" "$TARGET:$P" "$CURRENT" "$TOTAL"
        if (echo >/dev/tcp/"$TARGET"/"$P") 2>/dev/null; then
            BANNER=$(grab_banner "$TARGET" "$P")
            printf "\r\033[K"  # clear progress line before printing result
            if [ -n "$BANNER" ]; then
                echo -e "${GREEN}[OPEN]${RESET}   $P/$SVC — ${CYAN}$BANNER${RESET}"
            else
                echo -e "${GREEN}[OPEN]${RESET}   $P/$SVC"
            fi
            OPEN=$((OPEN+1))
        fi
    done
